...
import { Container, Dropdown, DropdownButton, Form, InputGroup, Navbar, OverlayTrigger, Pagination, Tooltip } from "react-bootstrap";
...
function HighlightedSearchItem(dto: {
  option: any;
  query: string;
}) {
...
const HomePageSearchBar = memo(function HomePageSearchBar(dto: {
  ...
  return <InputGroup className="mb-3">
      <DropdownButton variant="outline-secondary" title={["Contains", "Similar", "LCS"][dto.searchType]} id="input-group-dropdown-1">
      ...
      <AsyncTypeahead delay={0} clearButton filterBy={filterBy} id="basic-typeahead-single" labelKey="name" onSearch={handleSearch} options={options} placeholder="원하는 대사를 검색하세요" isLoading={isLoading}
    // className="me-2"
    open={open} paginationText="더보기" minLength={0} onKeyDown={onKeyDown} promptText="검색어를 입력하세요" onChange={onChange} renderMenuItemChildren={(option, props) => <Fragment>
            <HighlightedSearchItem option={option} query={query} />
          </Fragment>} />
    </InputGroup>;
});
const PaginationBar = memo(function PaginationBar(dto: {
  curPage: number;
  setCurPage: (page: number) => void;
}) {
  if (dto.curPage <= 5) {
    return <Pagination>
        <Pagination.Prev disabled={dto.curPage === 1} onClick={_ => dto.setCurPage(dto.curPage - 1)} />

        {range(1, 5).map(e => {
        return <Pagination.Item key={e} active={e === dto.curPage} onClick={_ => dto.setCurPage(e)}>
              {e}
            </Pagination.Item>;
      })}
        ...
        <Pagination.Next disabled={dto.curPage === 50} onClick={_ => dto.setCurPage(dto.curPage + 1)} />
      </Pagination>;
...
const range = (start: number, stop: number, step: number = 1) => Array.from({
  ...
  return <div>
      ...
      <Navbar>
        ...
        display: "block"
          ...
          <Form>
            <HomePageSearchBar setKeyword={setKeyword} searchType={searchType} setSearchType={setSearchType} />
          </Form>
        </Container>
      </Navbar>
