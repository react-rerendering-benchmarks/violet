...
import { dummyMessageRaw, SearchMessageRawResultType, SearchMessageRectangleType } from "../../utils/searchMessage";
function TestSearchResultImage(dto: {
  ...
  useEffect(() => {
    axios.get(`/imageurl/${dto.result.ArticleId}/${dto.result.Page + 1}`).then(e => {
  ...
  return <>
      ...
      position: "relative"
          ...
          {dto.rects.map(e => <div className="overlay" title={e.MessageRaw} style={{
        position: "absolute",
...
export default memo(function TestPage() {
  ...
  return <div>
      <TestSearchResultImage result={({
      ArticleId: dummyMessageRaw[0].ArticleId,
      Page: parseInt(page!)
    } as SearchMessageRawResultType)} rects={dummyMessageRaw.filter(e => e.Page === parseInt(page!))} />
    </div>;
});